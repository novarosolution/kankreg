...
export default function KankregKpiStrip({ items = [] }) {
  ...
  const cols = isXs ? 1 : Math.min(items.length, 3);

  return (
    <View style={[styles.row, isXs && styles.rowStack]}>
      {items.map((item, idx) => (
        <SectionReveal key={item.key} index={idx} preset="fade-up" delay={idx * 40}>
          <Pressable
            onPress={item.onPress}
            disabled={!item.onPress}
            style={({ pressed, hovered }) => [
              styles.kpi,
              { width: isXs ? "100%" : `${100 / cols - 2}%`, flex: isXs ? undefined : 1 },
