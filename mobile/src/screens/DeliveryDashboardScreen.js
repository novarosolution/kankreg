...
import {
  ...
  updateDeliveryLocation} from "../services/deliveryService";
...
import KankregDeliveryLayout from "../components/kankreg/KankregDeliveryLayout";
...
export default function DeliveryDashboardScreen({ navigation }) {
  ...
  const pushLocation = useCallback(
    async (coords) => {
      ...
      try {
        ...
        setLocError(e.message || "Could not send location.");
      }
    },
    [token]
  );
  ...
  const startWatching = useCallback(async () => {
    ...
    try {
      ...
      if (status !== "granted") {
        setLocError("Location permission denied. Enable it in system settings.");
        setShareLiveLocation(false);
        return;
      }
      ...
      setLocError(e.message || "Could not start location.");
      setShareLiveLocation(false);
    }
  ...
  const activeMapOrderId = useMemo(() => {
    const list = Array.isArray(orders) ? orders : [];
    const inTransit = list.find(
      (o) =>
        o.status === "ready_for_pickup" ||
        o.status === "shipped" ||
        o.status === "out_for_delivery"
    );
    return inTransit?._id || list[0]?._id || null;
  }, [orders]);
      ...
      <CustomerScreenShell style={styles.screen}>
        ...
        >
          <KankregUnifiedPageHeader
            ...
            title="Delivery"
            subtitle="Your delivery runs"
          />
          <View style={[styles.panel, customerPanel(c, shadowPremium, isDark)]}>
            <PremiumEmptyState
              iconName="bicycle-outline"
              title="No delivery access"
              description="Your account is not enabled as a delivery partner. Ask an admin to turn on delivery access in Manage Users."
    ...
    <CustomerScreenShell style={styles.screen}>
      ...
      >
        <KankregUnifiedPageHeader
          ...
          subtitle="Assigned orders & status"
          navigation={navigation}
          right={
            <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
              <PremiumChip label="● Online" tone="success" size="sm" />
        ...
        <View style={[styles.panel, customerPanel(c, shadowPremium, isDark)]}>
          <KankregDeliveryLayout
            activeOrderId={activeMapOrderId}
            main={
          <>
          <PremiumCard variant="muted" padding="md" style={styles.liveLocationCard}>
            <View style={styles.liveLocationRow}>
              ...
              <Switch
                accessibilityLabel={DELIVERY_LIVE_SHARE.switchA11yLabel}
                ...
                thumbColor={shareLiveLocation ? c.primary : c.surfaceMuted}
              />
            </View>
            {Platform.OS === "web" ? (
          ...
          {!loading && orders.length === 0 ? (
            <PremiumEmptyState
              ...
              description="You will see active assigned orders here (confirmed through out for delivery)."
              compact
            />
          ) : null}
