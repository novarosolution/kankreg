...
function PremiumInputBase({
  ...
  const borderColor = hasError
    ? c.danger
    : focused
      ? isDark ? ALCHEMY.goldBright : ALCHEMY.gold
      : c.border;
  ...
  const labelColor = hasError ? c.danger : focused ? (isDark ? ALCHEMY.goldBright : ALCHEMY.brown) : c.textMuted;
          ...
          <View style={styles.iconLeftWrap}>
            {typeof iconLeft === "string" ? (
              <Ionicons
                name={iconLeft}
                size={icon.sm}
                color={focused ? (isDark ? ALCHEMY.goldBright : ALCHEMY.brown) : c.textMuted}
              />
            ) : (
              iconLeft
            )}
